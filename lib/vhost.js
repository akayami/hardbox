...
module.exports = class vhost {
	...
	requireModule(module) {
		try {
			...
			console.debug('Loading module via global: ' + module);
			const gpath = path.resolve(this.globalPath, module);
			return require(gpath);
		}
	}
	...
	initializeModules() {
		if (this.config.modules) {
			for (let x = 0; x < this.config.modules.length; x++) {
				try {
					const module = this.requireModule(this.config.modules[x].name);
					module(
						this.app,
